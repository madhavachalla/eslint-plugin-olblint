...
module.exports = {
    meta: {
        docs: {
            description: "Custom JSDoc Validation",
            category: "Fill me in",
            recommended: false
        },
    ...
    create: function(context) {
          ...
          function checkForPrototypeExpression (node) {
            if (node.object && node.object.type === "MemberExpression") {
                if (node.object.property.name === "prototype") {
                    return true;
                }
            }
            return false;
            ...
            'FunctionExpression': function(node){
              ...
              if (node.parent) {
                if (node.parent.type === 'AssignmentExpression') {
                   if (checkForPrototypeExpression (node.parent.left)) {
                    commentNode = node.parent.parent;
                    refNode = node.parent.left;
                    key = 'property';
                   }    
                   else {
                       return true;
